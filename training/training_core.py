...
def train_step(
    ...
    if r > 0:
        if FLAGS.config.gr_warmup_strategy == "r":
            r = r_fn(FLAGS.config.gnp.r, step)
        if FLAGS.config.gr_warmup_strategy == "lambda":
            alpha = r_fn(FLAGS.config.gnp.alpha, step)
...
def train_for_one_epoch(
    ...
    steps_per_epoch = int(math.floor(dataset_source.num_training_obs / dataset_source.batch_size))
