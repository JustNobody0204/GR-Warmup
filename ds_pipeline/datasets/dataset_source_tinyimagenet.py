...
class TinyImagenet(dataset_source.DatasetSource):
  ...
  def __init__(self, batch_size: int, image_size: int,
    ...
    self.train_data_info = get_directory_info(FLAGS.config.tinyimagenet_train_dir)
    self.eval_data_info = get_directory_info(FLAGS.config.tinyimagenet_val_dir)
    self.load_files()
  ...
  def decode_example(self, example: Dict[str, tf.Tensor],
    ...
    Returns:
      A dictionary with an 'image' tensor and a one hot encoded 'label' tensor.
    """
